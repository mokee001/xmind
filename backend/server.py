...
from . import unified_selection
from .routers import selection as selection_routes
...
async def _make_wall(template_id: str, title: str, date: str, photos: list[dict] | None = None,
                     ...
                     scope: str = "legacy", preference_revision_id: str = "",
                     selection_contract: str = "", selection_sources: list[str] | None = None) -> dict | None:
    ...
    unified = selection_contract == unified_selection.CONTRACT
    if selection_contract and not unified:
        raise HTTPException(409, '不支持的选片链路版本')
    selection_provenance = None
    if unified:
        try:
            selection_scope = unified_selection.library_scope(scope, selection_sources or ['all'])
            selection_snapshot = unified_selection.candidates(selection_scope)
            photos = selection_snapshot['photos']
        except unified_selection.SelectionUnavailable as error:
    ...
    photos = photos if unified else curation.prepare(photos)
    ...
    if applied_filters and not unified:
    ...
    if unified:
        try:
            chosen, selection_provenance = unified_selection.select(
                selection_scope, slot_n, prefer=applied_filters, exclude=excluded, rotate=rotate, avoid=avoid,
    ...
    if not unified and not applied_filters and slot_n >= 2 and len(photos) > slot_n:
    ...
    wall = {
        ...
        "chosen": [{"filename": c["filename"], "final_score": c.get("final_score"),
...
async def generate(req: GenerateReq, x_account_token: str = Header(default="")) -> dict:
    ...
    wall = await _make_wall(
        ...
        selection_contract=req.selection_contract,
        selection_sources=req.selection_sources,
    )
    if wall is None:
...
selection_routes.register(app, _account_scope,
                          lambda token:any(_account_auth(device,token) for device in _devices().values()))
