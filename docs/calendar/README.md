...
- 默认可通过本机 Ollama 使用 `qwen3-vl:4b-instruct` 生成同一 Schema 的处理决策，不需要 API Key，照片不离开设备。
...
完全本地运行参见 [`本地小模型运行说明.md`](本地小模型运行说明.md)。本地后端与云端后端共用 Prompt、规则、校准案例、Pydantic Schema 和渲染链，模型切换不会改变最终文件格式。
