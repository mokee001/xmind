...
export default function DeviceSetupFlow({
  ...
  const discoverDevices = async () => {
    ...
    try {
      ...
      const found = await adapter.startDeviceDiscovery();
      setDevices(Array.isArray(found) ? found : []);
    } catch (caught) {
  ...
  const chooseDevice = async device => {
    ...
    try {
      ...
      await adapter.connectProvisioningDevice(device.deviceId);
  ...
  const connectWifi = async () => {
    ...
    try {
      ...
      const provisioned = await adapter.provisionWifi({ ssid: selectedNetwork.ssid, password });
      setProgress(100);
      setStatusText('连接完成');
    ...
    if (stage === 'wifi') {
      return (
        <>
          ...
          {busy ? <Text style={styles.scanningText}>正在读取附近网络…</Text> : null}
  ...
  const header = (
    <View style={styles.sheetHeader}>
      ...
      {!embedded ? <Pressable onPress={onClose} style={styles.closeButton}><Text style={styles.closeText}>×</Text></Pressable> : null}
    </View>
  );
  const body = (
    <Animated.View style={{
